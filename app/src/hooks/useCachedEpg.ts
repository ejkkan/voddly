    ...
    queryFn: async () => {
      ...
      if (data?.epg_listings) {
        ...
        return listings.map((p: any) => ({
          ...
          end: p.end || p.stop || new Date(p.stop_timestamp * 1000).toISOString(),
        }));
      }
  ...
  const epgData = useMemo(() => {
    ...
    channelIds.forEach(channelId => {
      if (!result.has(channelId) && fetchedChannelsRef.current.has(channelId)) {
        ...
        const cachedData = queryClient.getQueryData(['channel-epg', sourceId, channelId]) as any;
        if (cachedData) {
...
export function useSimpleVirtualizedEpg(
  sourceId: string | undefined,
  allChannels: Array<{ id: string; sourceItemId?: string }>,
  ...
  const channelsToFetch = useMemo(() => {
    ...
    const visibleChannels = allChannels.slice(visibleRange.start, visibleRange.end);
  ...
  const { epgData: newEpgData, isLoading, isError } = useBatchedEpg(
    sourceId,
    channelsToFetch
  );
    ...
    allChannels.forEach(channel => {
      ...
      if (!channelPrograms) {
        ...
        if (cachedData) {
          ...
          channelPrograms = Array.isArray(cachedData) ? cachedData : (cachedData as any)?.programs || [];
        }
      }
      ...
      if (channelPrograms && channelPrograms.length > 0) {
        ...
        const sortedPrograms = [...channelPrograms].sort((a: any, b: any) => 
