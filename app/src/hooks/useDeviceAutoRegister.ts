...
export function useDeviceAutoRegister() {
  ...
  useEffect(() => {
    const checkAndSetupDevice = async () => {
      try {
        ...
        if (session.data.accountId && currentAccountId !== session.data.accountId) {
          log.info('[DeviceAutoRegister] Account changed, resetting device state');
          await secureSession.setDeviceRegistered(false, session.data.accountId);
        }
        ...
        if (!isRegistered && !hasPassphrase && session.data.accountId) {
          log.info('[DeviceAutoRegister] Device needs registration and passphrase');
          // The app should redirect to passphrase setup
        }
      } catch (error) {
        log.error('[DeviceAutoRegister] Error checking device status', { error });
      }
    };
  ...
  return {
    markDeviceRegistered: async (accountId: string) => {
      ...
      log.info('[DeviceAutoRegister] Device marked as registered', { accountId });
    },
