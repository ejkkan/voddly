...
export function useWatchStateTracker(params: UseWatchStateTrackerParams) {
  const { profileId, contentId, contentType, reportIntervalMs = 10_000, playerPreferences } = params;
  const queryClient = useQueryClient();
  const [currentPreferences, setCurrentPreferences] = useState<PlayerPreferences>(playerPreferences || {});
  ...
  useEffect(() => {
    if (data?.state) {
      setCurrentPreferences(prev => ({
        playbackSpeed: prev.playbackSpeed || data.state.playback_speed || undefined,
        audioTrack: prev.audioTrack || data.state.audio_track || undefined,
        subtitleTrack: prev.subtitleTrack || data.state.subtitle_track || undefined,
        qualityPreference: prev.qualityPreference || data.state.quality_preference || undefined,
      }));
    }
  }, [data?.state]);
  ...
  const updateMutation = useMutation({
    mutationFn: async (params: {
      ...
      const isCompleted = completed || 
        (durationSec && positionSec >= durationSec * 0.95);
      ...
      console.log('[useWatchStateTracker] Mutation - merged preferences:', mergedPreferences);
      console.log('[useWatchStateTracker] Mutation - sending payload:', JSON.stringify(updatePayload, null, 2));
  ...
  const onProgress = useCallback<NonNullable<WatchCallbacks['onProgress']>>(
    (currentTimeSec, durationSec) => {
      ...
      if (durationSec && currentTimeSec >= durationSec * 0.95 && !completedRef.current) {
        completedRef.current = true;
  ...
  >(
    (currentTimeSec, durationSec) => {
      ...
      const isNearEnd = lastDurationRef.current && 
        lastTimeRef.current >= lastDurationRef.current * 0.95;
      void sendUpdate(lastTimeRef.current, lastDurationRef.current, isNearEnd || completedRef.current);
  ...
  const updatePreferences = useCallback(
    async (newPrefs: Partial<PlayerPreferences>) => {
      ...
      console.log('[useWatchStateTracker] Updated preferences to:', updatedPrefs);
      ...
      console.log('[useWatchStateTracker] Sending immediate update after preference change');
      ...
      try {
        ...
        console.log('[useWatchStateTracker] Preference update sent successfully');
      } catch (error) {
        console.error('[useWatchStateTracker] Error sending preference update:', error);
      }
    },
    [updateMutation]
  );
