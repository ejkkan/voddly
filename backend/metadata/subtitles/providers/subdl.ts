...
export class SubDLProvider extends BaseSubtitleProvider {
  ...
  async downloadSubtitle(
    ...
    // source_id might be a full path like "/subtitle/3496698-8419047.zip" or just an ID
    let downloadUrl: string;
    if (metadata.source_id.startsWith('/')) {
      // Full path provided
      downloadUrl = `https://dl.subdl.com${metadata.source_id}`;
    } else {
      // Just ID provided
      downloadUrl = `https://dl.subdl.com/subtitle/${metadata.source_id}.zip`;
    }
    ...
    if (metadata.release) {
      const normalizedRelease = metadata.release
        .toLowerCase()
        .replace(/\s+/g, '');
