...
export class OpenSubtitlesProvider extends BaseSubtitleProvider {
  ...
  async downloadSubtitle(
    ...
    if (!this.apiKey) {
      throw new Error('OpenSubtitles API key not provided');
    }
    ...
    const response = await fetch(
      'https://api.opensubtitles.com/api/v1/download',
      {
        ...
        body: JSON.stringify({
          file_id: parseInt(metadata.source_id),
        }),
      }
    );
    ...
    const downloadData = (await response.json()) as { link: string };
