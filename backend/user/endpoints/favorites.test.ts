...
describe('Favorites endpoints', () => {
  ...
  test('addFavorite inserts', async () => {
    ...
    const res = await addFavorite({ profileId: 'p1', contentUid: 'tmdb:movie:1' } as any);
  ...
  test('removeFavorite deletes', async () => {
    mockUserDB.exec.mockResolvedValue(undefined);
    const res = await removeFavorite({ profileId: 'p1', contentUid: 'tmdb:movie:1' } as any);
    expect(res.ok).toBe(true);
    expect(mockUserDB.exec).toHaveBeenCalledTimes(1);
  });
  ...
  test('listFavorites returns rows', async () => {
    mockUserDB.query.mockReturnValueOnce(
      makeAsyncIterable([{ content_uid: 'tmdb:movie:1', added_at: new Date('2024-01-01T00:00:00Z') }])
    );
